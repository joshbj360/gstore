                ...
                <div>
                    ...
                    <div v-for="(rate, index) in zone.rates" :key="index" class="p-4 border rounded-md mb-3 bg-gray-50/50 space-y-3">
                        <div class="flex items-end gap-3">
                            ...
                            <button @click="removeRate(index)" class="p-2 text-gray-400 hover:text-red-500">
                                <Icon name="mdi:trash-can-outline" size="20" />
                            </button>
                        </div>
                    </div>
                    <button @click="addRate" class="text-sm font-semibold text-[#f02c56] hover:underline">+ Add Rate</button>
                </div>
            </div>

            <div class="p-4 border-t bg-gray-50 flex justify-end gap-3">
                <button @click="$emit('close')" class="px-4 py-2 border rounded-lg text-sm font-semibold">Cancel</button>
                <button @click="$emit('save', zone)" class="px-4 py-2 bg-[#f02c56] text-white rounded-lg text-sm font-semibold">Save Profile</button>
            </div>
        </div>
    </div>
</template>
