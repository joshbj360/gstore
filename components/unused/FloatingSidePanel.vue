<template>
  <div class="fixed right-2 bottom-16 flex flex-col gap-2.5 z-50 lg:hidden bg-white p-2 rounded-lg shadow-lg">
    ...
    <NuxtLink
      to="/cart"
      class="relative w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center text-gray-600 hover:bg-brand/10 transition-all active:scale-95"
      aria-label="View cart"
    >
      <Icon name="mdi:cart" size="20" />
      <span
        v-if="cartStore.cartCount"
        class="absolute -top-1 -right-1 bg-brand text-white text-xs w-4 h-4 rounded-full flex items-center justify-center"
      >
        {{ cartStore.cartCount }}
      </span>
    </NuxtLink>
    ...
    <NuxtLink
        ...
        class="w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center text-gray-600 hover:bg-brand/10 transition-all active:scale-95"
      ...
      >
        ...
        <div class="flex justify-around">
          <a
            ...
            target="_blank"
            class="p-2 rounded-full hover:bg-brand/10"
        ...
        <button
          @click="isShareModalOpen = false"
          class="mt-4 w-full py-2 bg-brand text-white rounded-lg hover:bg-[#df4949]"
        >
          Close
        </button>
      </div>
