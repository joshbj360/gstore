<template>
  <div class="space-y-6 p-4 sm:p-6">
    ...
    <div v-if="loading" class="flex justify-center">
      <div class="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#f02c56]"></div>
    </div>
      ...
      <div v-if="activeSection === 'about'" class="bg-white shadow-md rounded-lg p-4 sm:p-6">
        <div class="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 mb-6">
          ...
          <div class="text-center sm:text-left">
            ...
            <p v-if="seller?.is_verified" class="text-[#f02c56] font-medium">Verified Seller</p>
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <p v-if="seller?.store_website" class="text-sm sm:text-base">
              ...
              <a :href="seller?.store_website" class="text-[#f02c56] hover:underline" target="_blank">{{ seller?.store_website }}</a>
            </p>
          </div>

          <div v-if="seller?.store_socials" class="flex space-x-4">
            <a v-for="(link, key) in seller.store_socials" :key="key" :href="link" target="_blank" class="text-[#f02c56] hover:text-[#d81b46] transition-all duration-250">
          ...
          <div v-if="seller?.ratings">
            ...
            <p class="text-[#f02c56] text-base sm:text-lg">
      ...
      <div v-if="activeSection === 'products'" class="bg-white shadow-md rounded-lg p-4 sm:p-6">
        ...
        <div v-if="seller?.products?.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          >
            ...
            <p class="text-[#f02c56] font-semibold mt-2 text-sm sm:text-base">${{ product.price.toFixed(2) }}</p>
            <NuxtLink
              :to="`/product/${product.slug}`"
              class="mt-4 inline-block px-3 py-2 sm:px-4 sm:py-2 bg-[#f02c56] text-white rounded-md hover:bg-[#d81b46] transition-all duration-250 text-sm sm:text-base"
            >
              View Product
            </NuxtLink>
          </div>
        ...
        <div v-if="seller?.reviews?.length" class="space-y-4">
          ...
          >
            <div class="flex items-center space-x-2">
              <p class="text-[#f02c56] text-sm sm:text-base">
