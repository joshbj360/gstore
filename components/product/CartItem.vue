<template>
  <div class="flex items-start gap-4">
    ...
    <div class="flex flex-col items-end gap-2">
      <button @click="cartStore.removeFromCart(item.variantId)" class="p-1 text-gray-400 dark:text-neutral-500 hover:text-brand-dark dark:hover:text-brand-light">
...
const updateQuantity = (quantity: number) => {
  if (quantity > 0) {
    // THE FIX: This now calls the store with item.variantId
    cartStore.updateItemQuantity(props.item.variantId, quantity);
  }
};
</script>