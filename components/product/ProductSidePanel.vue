<template>
  <div class="fixed right-2 bottom-16 flex flex-col gap-2.5 z-50 lg:hidden">
    <!-- Details Button (Product-specific) -->
    <button v-if="currentProduct" @click="emit('toggle-details', currentProduct.id)"
    ...
    <!-- Like Button (Product-specific) -->
    <button v-if="currentProduct" @click="toggleLike"
      ...
      :class="{ 'bg-[#f02c56] text-white': isLiked }" aria-label="Like product">
    ...
    <!-- Share Button (Product-specific) -->
    <button v-if="currentProduct" @click="openShareModal"
      class="w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center text-gray-600 hover:bg-[#f02c56]/10 transition-all active:scale-95"
  ...
  <transition enter-active-class="transition-opacity duration-200 ease-out"
    ...
    <div v-if="isShareModalOpen && currentProduct"
      class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click="isShareModalOpen = false">
      <div class="bg-white rounded-lg p-4 mx-4 w-full max-w-sm" @click.stop>
        ...
        <div class="flex justify-around">
          <a :href="`https://www.tiktok.com/share?url=${encodeURIComponent(shareUrl)}`" target="_blank"
            class="p-2 rounded-full hover:bg-[#f02c56]/10" aria-label="Share on TikTok">
            <Icon name="mdi:tiktok" size="24" />
          </a>
          <a :href="`https://www.instagram.com/?url=${encodeURIComponent(shareUrl)}`" target="_blank"
            class="p-2 rounded-full hover:bg-[#f02c56]/10" aria-label="Share on Instagram">
          ...
          <a :href="`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`" target="_blank"
            class="p-2 rounded-full hover:bg-[#f02c56]/10" aria-label="Share on Facebook">
...
const currentProduct = computed(() => {
  return productStore.products.find(p => p.id === (props.currentProductId !== null ? Number(props.currentProductId) : null)) || null;
});
