<template>
    <div id="product-card"
        class="bg-white dark:bg-neutral-900 rounded-xl shadow-md overflow-hidden border border-gray-200 dark:border-neutral-800 transition-all duration-300 hover:shadow-lg">
        ...
        <NuxtLink :to="`/product/${product.slug}`"
            class="block w-full aspect-square relative overflow-hidden group bg-gray-100 dark:bg-neutral-800">
            <img :src="getMediaThumbnailUrl(product.media?.[0])"
                :alt="`Product image ${product?.media[0]?.altText || ''}`"
                class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" :width="500"
                :height="500" />
            <div v-if="product.media?.[0]?.type === 'VIDEO'"
        ...
        <div class="p-3">
            <h3 class="text-sm font-semibold text-gray-800 dark:text-neutral-100 line-clamp-2 leading-tight h-10">{{
                product.title }}</h3>
                ...
                <div class="relative">
                    ...
                    <transition enter-active-class="transition-all duration-200 ease-out"
                        ...
                        <div v-if="showVariantSelector" v-click-outside="() => showVariantSelector = false"
                            ...
                            <p
                                class="text-xs font-semibold text-gray-500 dark:text-neutral-400 px-2 py-1.5 border-b border-gray-200 dark:border-neutral-700 shrink-0">
                                Select Size</p>
