<template>
    <div id="product-card" class="bg-neutral-900 rounded-xl shadow-md overflow-hidden border border-neutral-800 transition-all duration-300 hover:shadow-lg">
        
        <!-- LIKE BUTTON -->
        <button 
            @click.stop="likeStore.toggleProductLike(product.id!)"
            class="absolute top-2 right-2 z-20 h-8 w-8 bg-black/30 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:text-[#f02c56] transition-colors"
        ...
        <NuxtLink :to="`/product/${product.slug}`" class="block w-full aspect-square relative overflow-hidden group">
            <img 
        ...
        <!-- Product details -->
        <div class="p-3">
            <h3 class="text-sm font-semibold text-neutral-100 line-clamp-2 leading-tight h-10">{{ product.title }}</h3>
            
            <div class="flex justify-between items-center mt-1">
                <p class="text-base font-bold text-neutral-100">{{ formatPrice(product.price) }}</p>

                <!-- ADD TO CART BUTTON & VARIANT POP-UP -->
                <div class="relative">
                    <button 
                        @click.stop="handleAddToCartClick"
                        class="h-9 w-9 flex items-center justify-center rounded-full bg-neutral-800 hover:bg-brand hover:text-white text-neutral-300 transition-colors"
                    ...
                    >
                        <div v-if="showVariantSelector" v-click-outside="() => showVariantSelector = false" 
                             class="absolute bottom-full mb-2 right-0 bg-neutral-800 rounded-lg shadow-2xl border border-neutral-700 w-36 max-h-48 flex flex-col z-30">
                            <p class="text-xs font-semibold text-neutral-400 px-2 py-1.5 border-b border-neutral-700 shrink-0">Select Size</p>
                            <div class="mt-1 space-y-1 p-1 overflow-y-auto">
                                <button 
                                    ...
                                    class="w-full text-left px-2 py-1.5 text-sm font-medium rounded-md text-neutral-100 hover:bg-neutral-700 disabled:opacity-50 disabled:line-through"
                                >
                                    {{ variant.size }}
                                </button>
                            </div>
