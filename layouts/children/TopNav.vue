<template>
  <nav id="top-nav" class="fixed bg-white z-50 w-full border-b border-gray-200 h-16" role="navigation">
    <div class="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 h-full">
      ...
      <!-- Search Bar -->
      <div v-click-outside="() => isSearchFocused = false" class="flex-1 max-w-lg mx-4 relative">
        ...
        >
          <div v-if="isSearchFocused && (searchResults.length || isSearching || searchItem)" class="absolute bg-white w-full mt-2 shadow-lg rounded-lg z-50 border max-h-96 overflow-y-auto">
            ...
            <div v-else-if="!searchResults.length && searchItem" class="p-4 text-center text-sm text-gray-500">No results found for "{{ searchItem }}"</div>
            <div v-else>
      ...
      <div class="flex items-center gap-2 sm:gap-4">
        <div v-if="!userStore.isLoggedIn" class="flex items-center gap-2"> 
          <button @click="router.push('/auth/login')" class="flex items-center bg-[#f02c56] text-white rounded-lg px-4 py-2 text-sm font-medium hover:bg-[#d81b36] transition-all">
            Log in
          </button>
        </div>
