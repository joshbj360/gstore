  ...
  <div>
    ...
    <div v-else class="p-6 md:p-8 max-w-6xl mx-auto">
      ...
      <div class="bg-white rounded-xl shadow-sm p-6 mb-8">
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div class="flex gap-3">
            <button @click="openChat" class="px-5 py-2.5 rounded-lg font-medium text-sm border border-[#C42B78] text-brand-dark hover:bg-brand/10 transition-colors flex items-center">
              <Icon name="mdi:message-outline" size="18" class="mr-2" />
              Chat
            </button>
            <button v-if="userStore.isLoggedIn && userStore.user?.id !== sellerStore.profileId" @click="toggleFollow" class="px-5 py-2.5 rounded-lg font-medium text-sm border transition-colors w-28" :class="{ 'border-[#C42B78] text-brand-dark hover:bg-brand/10': !isFollowing, 'bg-brand text-white border-[#C42B78]': isFollowing }">
      ...
      <div class="flex items-center border-b border-gray-200 mb-8">
        <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id" class="px-4 py-3 font-medium text-sm relative" :class="{'text-brand-dark': activeTab === tab.id, 'text-gray-600 hover:text-gray-900': activeTab !== tab.id}">
          {{ tab.label }}
          <span v-if="activeTab === tab.id" class="absolute bottom-0 left-0 right-0 h-0.5 bg-brand"></span>
        </button>
      </div>
      ...
      <div v-if="activeTab === 'about'" class="bg-white rounded-xl shadow-sm p-6">
        <div class="prose prose-sm text-gray-600 max-w-none">
          ...
          <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6 not-prose">
            <div>
                <h3 class="font-medium text-gray-800 mb-2 flex items-center gap-2"><Icon name="mdi:calendar-month" size="18" class="text-brand-dark" />Store Since</h3>
                <p>{{ formatDate(sellerStore.created_at) }}</p>
            </div>
            <div>
                <h3 class="font-medium text-gray-800 mb-2 flex items-center gap-2"><Icon name="mdi:map-marker-outline" size="18" class="text-brand-dark" />Location</h3>
