  ...
  <HomeLayout>
    ...
    <div>
      ...
      <div v-else-if="error || !sellerData" class="text-center py-20">
        ...
        <p class="text-neutral-400 mt-2">The store you are looking for does not exist or may have been moved.</p>
        <NuxtLink to="/" class="mt-4 inline-block bg-brand text-white px-6 py-2 rounded-md hover:bg-[#d81b36]">
            Browse All Products
        </NuxtLink>
      </div>

      <div v-else class="text-neutral-100">
        <!-- Cover Image -->
        <div class="relative h-48 bg-neutral-800">
          <img :src="sellerData.store_banner || 'https://placehold.co/1500x500/171717/333333?text=Banner'" :alt="`${sellerData.store_name} cover`" class="w-full h-full object-cover" />
          <div class="absolute inset-0 bg-gradient-to-t from-neutral-950/50 to-transparent" />
        </div>
        ...
        <div class="px-4 max-w-6xl mx-auto">
          ...
          <div class="flex items-end gap-4 -mt-12 relative z-10">
            <img :src="sellerData.store_logo || '/default-store-logo.png'" :alt="sellerData.store_name || 'Store Logo'" class="w-24 h-24 rounded-full border-4 border-neutral-900 shadow-lg object-cover" />
            <div class="flex-1 pb-2">
              <div class="flex items-center gap-2 mb-1">
                <h2 class="text-2xl font-bold text-neutral-100">{{ sellerData?.store_name }}</h2>
          ...
          <!-- Stats Bar -->
          <div class="flex justify-around py-4 border-y border-neutral-800 my-4">
            <div class="text-center"><div class="font-bold text-lg">{{ sellerProducts.length }}</div><div class="text-sm text-neutral-400">Products</div></div>
            <div class="text-center"><div class="font-bold text-lg">{{ formatNumber(sellerData.followers_count || 0) }}</div><div class="text-sm text-neutral-400">Followers</div></div>
            <div class="text-center"><div class="font-bold text-lg">4.8 <span class="text-neutral-500 font-normal">(1.2k)</span></div><div class="text-sm text-neutral-400">Rating</div></div>
          </div>

          <!-- Action Buttons -->
          <div class="flex gap-3 py-4">
            <button @click="toggleFollow" class="flex-1 py-2.5 rounded-lg font-semibold text-sm transition-colors" :class="isFollowing ? 'bg-neutral-700 text-neutral-100' : 'bg-brand text-white'">
              {{ isFollowing ? 'Following' : 'Follow' }}
            </button>
            <button @click="openChat" class="flex-1 py-2.5 rounded-lg font-semibold text-sm border border-neutral-700 bg-neutral-800 hover:bg-neutral-700">Message</button>
          </div>
        ...
        <div class="p-4 max-w-6xl mx-auto">
          <div v-if="activeTab === 'products'">
            ...
            <div v-else class="text-center py-12 text-neutral-500">This store has no products yet.</div>
          </div>
          <div v-else-if="activeTab === 'about'" class="prose prose-invert max-w-none text-neutral-300">
...
const toggleFollow = () => { /* ... follow logic ... */ };
const openChat = () => { /* ... chat logic ... */ };
</script>

<style scoped>
.tab-button {
    @apply flex-1 py-3 text-center font-medium border-b-2 transition-colors text-neutral-500 border-transparent hover:text-neutral-100;
}
.tab-button.active {
    @apply border-brand text-[#f02c56];
}
</style>
